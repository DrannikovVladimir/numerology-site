...
- **Реальный запуск в продакшн выполнен (20 июля 2026)**:
  - Сервер полностью сброшен — `content/pending/` и `content/published/`
    очищены, `planned-urls.json` пересобран с нуля (0 из 180
    опубликовано)
  - 34 готовые статьи синхронизированы на сервер (`sync-content.sh`)
  - Опубликовано 30 статей (`publish.js --count 30` + `linkbuilder.js`),
    4 статьи намеренно оставлены в `content/pending/` как буфер —
    при боевом расписании (1/день) хватит на 4 дня, пока пишется
    новый контент
  - Cron переключён с тестового режима (`*/2 часа`) на боевой:
    `0 10 * * *` (1 статья в день, 10:00 UTC)
  - `X-Robots-Tag: noindex` убран из Nginx — индексация открыта.
    `site/app/robots.ts` проверен, забытых `Disallow: /` нет
  - Sitemap проверен, отдаёт `200 OK`
  - **Важно:** очередь `content/pending/` небольшая (4 статьи на
    момент запуска) — нужно пополнять буфер новыми статьями, иначе
    ежедневная публикация остановится

## Не сделано / следующие шаги

**Пополнение буфера статей** — на момент запуска в продакшн (20 июля
2026) в очереди осталось всего 4 статьи, при расписании 1/день этого
хватит на 4 дня. Нужно писать новые статьи регулярно, чтобы очередь
не пустела — иначе ежедневная публикация остановится сама (без
ошибки, `publish.js` просто выходит, если `pending/` пуст).
