...
**Для spoke:**
- Ссылка вверх на родительский hub (из hub_id, конвертированного в дефисы —
  см. правила заполнения ниже)
- Анкор ссылки вверх (internal_links.up.anchor) — найди в semantic_clusters.json
  кластер с page_type "hub" и url, соответствующим hub_id (например hub_id
  "chislo-sudby" → url "/chislo-sudby/"), и возьми его поле title ДОСЛОВНО,
  с тем же регистром. Не придумывай текст анкора заново и не меняй регистр —
  иначе хлебная крошка на хаб будет писаться по-разному в разных статьях
  одной серии.
...
- Анкор ссылки вверх — та же логика, что и для spoke (см. выше): дословно
  из title хаба в semantic_clusters.json
...
- hub_id — в semantic_clusters.json это поле хранится через подчёркивание
  (например "chislo_sudby"). Перед вставкой в карточку замени ВСЕ подчёркивания
  на дефисы: "chislo_sudby" → "chislo-sudby". Не копируй значение буквально.
- cluster_keywords — если в semantic_clusters.json поле пустое ([]),
  сгенерируй 3-5 вариантов запроса на основе primary_keyword:
  синонимы, смежные формулировки, вопросные формы. Не копируй primary_keyword дословно.
...
Выдавай строго в таком порядке — без лишнего текста:

1. Одна строка резюме:
✓ Тип: [hub/spoke/standalone] | Шаблон: [template или null] | Тип числа/пары: [если применимо] | URL: [/url/]

2. JSON карточки — сразу, без заголовков и пояснений